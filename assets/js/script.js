  ...
  async function main() {
    ...
    function checkBadWords(validInputStr, invalidStorage, patternSet, matchedList, type) {
      for (const pattern of patternSet) {
        ...
        if (pattern[0] == "/") {
          // 正则表达式
          let exp = new RegExp(pattern.slice(1, -1), "g");
          let match = tempValidInputStr.match(exp);
          let length = 0;
          if (!match) continue;
          ...
          let multiple_valid = true;
          let words = pattern.split("|");
          let pos_list = [],
            deleted_length = 0;
          for (let i = 0; i < words.length; i++) {
            let index = tempValidInputStr.indexOf(words[i]);
            if (index == -1) {
    ...
    if (matchedList.length === 0) {
      document.querySelector(
        "#matches"
      ).innerHTML = `<div>未找到违禁词！你可以试试<a href="/pages/FAQ1/1aliceblue" target="_blank">下列解决方案</a><br><iframe src="/pages/FAQ1/aliceblue_iframe" width="100%" height="300px" style="background:transparent"></iframe></div>`;
